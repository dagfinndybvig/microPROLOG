"""
Pre-rendered View Renderer for Tarski's World
Generates static views at multiple angles for smooth rotation.
"""
...
class PrerenderedViewRenderer:
    """Renderer using pre-rendered views at multiple angles."""
    ...
    def __init__(self, world_file: str, width: int = 1024, height: int = 768):
        ...
        for i in range(self.num_views):
            angle = (i / self.num_views) * 2 * math.pi
            view_surface = self._render_view_at_angle(angle)
            self.views.append(view_surface)
        
        print(f"Created {self.num_views} views")
    
    def _project_3d_to_2d(self, x, y, z, angle_y):
        """Simple isometric projection at given angle."""
    ...
    def _world_to_3d(self, world_x, world_y):
        """Convert world coordinates to 3D coordinates."""
        x = -(world_x - 4.5)
        z = (world_y - 4.5)
        return (x, z)
    
    def _render_view_at_angle(self, angle_y):
        """Render a complete view at the given angle."""
        ...
        for i in range(8):
            for j in range(8):
                ...
                projected = [self._project_3d_to_2d(*c, angle_y)[:2] for c in corners]
                pygame.draw.polygon(surface, color, projected)
                pygame.draw.polygon(surface, (50, 50, 50), projected, 1)
        
        # Collect objects with depths
        objects_with_depth = []
        ...
        for name, obj in self.objects.items():
            ...
            x, z = self._world_to_3d(world_x, world_y)
            
            size_map = {'small': 0.4, 'medium': 0.6, 'large': 0.8}
            size_3d = size_map.get(obj.size, 0.6)
            y = size_3d / 2
            
            screen_x, screen_y, depth = self._project_3d_to_2d(x, y, z, angle_y)
    ...
    def handle_input(self):
        ...
        if keys[pygame.K_LEFT]:
            self.current_view = (self.current_view - 1) % self.num_views
            pygame.time.wait(100)  # Debounce
        if keys[pygame.K_RIGHT]:
            self.current_view = (self.current_view + 1) % self.num_views
            pygame.time.wait(100)  # Debounce
        if keys[pygame.K_SPACE]:
            self.current_view = 2  # Reset to initial view
    
    def draw_ui(self):
        """Draw UI overlay."""
        font = pygame.font.Font(None, 24)
        
        text = font.render(f"World: {os.path.basename(self.world_file)} | Objects: {len(self.objects)} | View: {self.current_view+1}/{self.num_views}", 
                          True, (255, 255, 255))
        self.screen.blit(text, (10, 10))
        
        text = font.render("Arrow Keys: Rotate | Space: Reset | ESC: Quit", True, (255, 255, 255))
    ...
    def run(self):
        ...
        try:
            while running:
                ...
                # Display current pre-rendered view
                self.screen.blit(self.views[self.current_view], (0, 0))
                self.draw_ui()
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python render_views.py <world_file.pl>")
        print("\nExample:")
        print("  python render_views.py ../world1.pl")
        sys.exit(1)
    ...
    renderer = PrerenderedViewRenderer(world_file)
    renderer.run()
